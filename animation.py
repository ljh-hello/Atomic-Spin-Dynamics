...
def gen_spec_spinlattice():
    ...
    spinLattice[1,:]=np.array([0,-0.01,1])
    # print(spinLattice)
    return spinLattice
...
plt.xlim(-2, 5) 
plt.ylim(-2, 5)
Q=ax.quiver(x, y,z, u, v, w, color='b', pivot='middle',normalize=True)
...
def update(step):
    ...
    sz = spin[:,2]   
    Q.remove()
    Q=ax.quiver(x, y, z, sx,sy,sz,color='b',  pivot='middle',normalize=True)
    
    # Q=ax.quiver(x, y, z, sx,sy,sz, length=0.8,color='b', normalize=True)
    # Q.set_UVC(sx,sy,sz)
    return Q,

ani = FuncAnimation(fig, update,blit=False, frames=step,interval=1E-16,cache_frame_data=False)
# fig.tight_layout()
plt.show()
ani.save('./data/Dimer/FM_start/diag_nonlocal.gif', writer='imagemagick')
...
# plt.xlim(-2, 5) 
# plt.ylim(-2, 5)
# Q=ax.quiver(x,y,u, v, color='b', pivot='middle')
...
#     sz = spin[:,2]   
#     Q.remove()
#     Q=ax.quiver(x,y,sx,sy,color='b',  pivot='middle')
    
#     # Q=ax.quiver(x, y, z, sx,sy,sz, length=0.8,color='b', normalize=True)
#     # Q.set_UVC(sx,sy,sz)
#     return Q,

# ani = FuncAnimation(fig, update,blit=False, frames=100,interval=1E-16,cache_frame_data=False)
# # fig.tight_layout()
# plt.show()
