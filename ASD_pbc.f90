Module global
    ...
    real(DP)      ::     damping_onsite=0.1,non_scale=0.5
    !Set steps
    INTEGER,PARAMETER :: step=500
    INTEGER,PARAMETER :: tmod=1
...
module find_neighbor_list
    use global
    public :: find_neighbor,read_poscar,find_neighbor_pbc
    private
    contains
    ...
    !expand_cell
        ...
        latt_s=sqrt(sum(latt_const2,dim=2))
...
!find_neighbor
    ...
    subroutine expand_cell_pbc(sc_pos,latt_s)
      implicit NONE
      !expand_cell
      INTEGER(DP)             ::        n,ii,jj,kk,mm
      Real(DP), DIMENSION(3)       :: latt_s
  ...
  !expand_cell
    ...
    subroutine find_neighbor_pbc(NN,NL1)
      implicit none
      Real(DP), DIMENSION(3)       :: latt_s
      INTEGER(DP), DIMENSION(:),ALLOCATABLE :: NN
      INTEGER(DP), DIMENSION(:,:),ALLOCATABLE :: NL1
      ...
      call expand_cell_pbc(sc_pos,latt_s)
      call read_poscar(latt_const,atom_pos,Natom)
      ...
      do ii=1,Natom*Nx*Ny*Nz
          do jj=1,Natom*Nx*Ny*Nz
              ...
              r12=sqrt(dot_product((sc_pos(ii,:)-sc_pos(jj,:))-nint((sc_pos(ii,:)-sc_pos(jj,:))/latt_s)*latt_s, &
              (sc_pos(ii,:)-sc_pos(jj,:))-nint((sc_pos(ii,:)-sc_pos(jj,:))/latt_s)*latt_s)) 
     ...
     function hamiltonian(spin,N_atome_tot,NN,NL1) result(energy)
      ...
      !different onsite damping
        elseif(damping_type==2) then
          damping=0.d0 
          damping(1,1,1) = damping_onsite*0.3
          damping(2,2,1) = damping_onsite*0.05
          damping(3,3,1) = damping_onsite
      !full onsite damping
...
program main
    use global
    use find_neighbor_list, ONLY: read_poscar,find_neighbor,find_neighbor_pbc
    ...
    do t=1,step
        ...
        IF(MOD(t,tmod) == 0) THEN
        write(10,"(7ES20.8)") time, E, norm2(M),s_update(:,1), DOT_PRODUCT(s_update(:,1),s_update(:,2))
        do iatom=1,N_atome_tot
