Module global
    ...
    real(DP)      ::     damping_onsite=0.1,non_scale=0.3
    !Set steps
    INTEGER,PARAMETER :: step=13000
    INTEGER,PARAMETER :: tmod=1
    integer,parameter :: Niter_midpoint = 100
    real(DP),parameter:: error_midpoint = 1E-8
    !
    real(DP) :: mi=1.d0, Gamma=41.39d0,dt=0.1d0,J_size=0.001d0,B_ext=0.d0
...
!STOP
       ...
       SUBROUTINE set_spin(s_new)
        IMPLICIT NONE
  
        !-- input
        real(DP),dimension(3,2) :: s_new
        !-- local
        real(DP) :: norm_s
  
        !initial guess of m^(-1) and m^(0)
         s_new(:,1) = (/0.57d0, 0.57d0,0.57d0/)
         norm_s = norm2(s_new(:,1))
         s_new(:,1) = mi/norm_s * s_new(:,1)
  
         s_new(:,2) = (/-0.57d0,-0.57d0,0.57d0/)
         norm_s = norm2(s_new(:,2))
         s_new(:,2) = mi/norm_s * s_new(:,2)
